...
def get_forutils():
    ...
    def git_install_forutils():
        try:
            ...
            fbranch = os.getenv(
                "FORUTILSBRANCH",
                "1.1.1" if os.environ.get("CONDA_BUILD") else "master",
            )
            try:
                if (
                    subprocess.call(
                        f"git clone --branch {fbranch} --depth=1 https://github.com/cmbant/forutils",
                        shell=True,
                    )
                    == 0
                ):
            ...
            if os.path.isdir(os.path.join("..", ".git")):
                try:
                    try:
                        os.chdir("..")
                        print(
                            "forutils directory found but no Makefile. "
                            "Attempting to clone submodule..."
                        )
                        if (
                            subprocess.call(
...
def remove_stale_dependency_files(build_dir, expected_forutils_path):
    ...
    for name in os.listdir(build_dir):
        ...
        if (
            "/forutils/" in dep_contents
            and normalized_forutils_path not in dep_contents
        ):
            os.remove(dep_path)
...
def make_library(cluster=False):
    ...
    if _compile.is_windows:
        ...
        if requested_compiler == "flang" or (
            not gfortran_ok and flang_command
        ):
            if not flang_command:
                raise RuntimeError(
                    "COMPILER=flang was requested, "
                    "but LLVM flang was not found on PATH."
                )

            compiler = flang_command
            compiler_version = _compile.call_command(
                f'"{compiler}" --version'
            )
            is_flang = True
            ...
            if not lld_link or not llvm_nm:
                raise RuntimeError(
                    "Windows builds with LLVM flang require "
                    "lld-link and llvm-nm on PATH to export "
                    "CAMB's Fortran symbols."
                )
        ...
        if ok and "8.2.0" in gfortran_version:
            print(
                "WARNING: gfortran 8.2.0 may be buggy and give "
                "unreliable results or crashes, upgrade gfortran."
            )
    ...
    if _compile.is_windows:
        if is_flang:
            ...
            flang_exe = shutil.which(
                compiler,
                path=_compile.compiler_environ.get("PATH"),
            )

            flang_bin_dir = (
                os.path.dirname(flang_exe) if flang_exe else None
            )

            libomp_dll = (
                os.path.join(flang_bin_dir, "libomp.dll")
                if flang_bin_dir
                else None
            )
            ...
            if libomp_lib and os.path.exists(libomp_lib):
                ...
                print(
                    "WARNING: could not locate libomp.lib next to "
                    "the flang executable; linking may fail to resolve "
                    "some OpenMP runtime symbols."
                )

        else:
            FFLAGS = (
                "-shared -static -cpp -fopenmp -O3 -fmax-errors=4"
            )
            link_flags = ""
            module_dir_flag = "-J"
        ...
        FORUTILS = [
            ...
            for f in _compile.makefile_dict(
                os.path.join(fpath, "Makefile")
            )["SRCS"]
        ...
        if is_flang:
            ...
            if not os.path.exists(omp_lib_mod):
                ...
                cmd = (
                    ...
                    + FFLAGS
                    + f" {shim_source} -c -o "
                    + f"{os.path.join(tmpdir, 'win_flang_omp_lib_shim.o')} "
                    + f"{module_dir_flag}{tmpdir}"
                )
        ...
        for source in FORUTILS + SOURCES:
            ...
            if modified:
                ...
                cmd = (
                    ...
                    + " "
                    + source
                    + f".f90{dependency_flag} -c -o {fout} "
                    + f"{module_dir_flag}{tmpdir}"
                )
        ...
        if need_compile or not os.path.exists(lib_file):
            if os.path.exists(lib_file):
                try:
                    ...
                    raise OSError(
                        "dll file in use. Stop python codes and "
                        "notebook kernels that are using camb."
                    )
            ...
            if is_flang:
                if libomp_dll and os.path.exists(libomp_dll):
                    ...
                    print(
                        "WARNING: could not locate libomp.dll "
                        "next to the flang executable; "
                        "the built cambdll.dll may fail to load "
                        "unless libomp.dll is on PATH."
                    )
        ...
        if not _compile.call_command("make -v"):
            raise OSError(
                ...
                'E.g. on ubuntu install with '
                '"sudo apt install make" '
                "(or use build-essential package)."
            )

        fpath = get_forutils()

        remove_stale_dependency_files(
            os.path.join(
                file_dir,
                "fortran",
                "Releaselib",
        ...
        if os.path.exists(lib_file) and not os.access(
            lib_file,
            os.W_OK,
        ):
            os.remove(lib_file)
        ...
        subprocess.call(
            f"make python "
            f"PYCAMB_OUTPUT_DIR={pycamb_path}/camb/ "
            f"CLUSTER_SAFE={cluster_safe}",
            shell=True,
        )

        if os.path.isfile(lib_file):
            subprocess.call(
                f"chmod 755 {lib_file}",
                shell=True,
            )
    ...
    if (
        not os.path.exists(tem)
        or os.path.getmtime(tem)
        < os.path.getmtime(tem_file)
    ):
