    ...
    function locked_h_ratio(a) result(ratio)
    use precision
    implicit none
    real(dl), intent(in) :: a
    real(dl) :: ratio, x, b0, b1, b2, t, raw, s
    integer :: j
    real(dl), parameter :: amin_fit = 2.04999999999999996e-04_dl
    real(dl), parameter :: amin_blend = 1.37350000000000005e-04_dl
    real(dl), parameter :: log_amin = -9.21034037197618183e+00_dl
    real(dl), parameter :: c(0:36) = (/ &
        & -1.88604945656052903e-02_dl, 6.39795466865536195e-03_dl, 5.00799242108293172e-03_dl, 4.04596490879708184e-03_dl, &
        & 2.34753685835038892e-03_dl, 1.40355942382533005e-03_dl, 3.79243492155354963e-04_dl, 2.11220811380838873e-05_dl, &
        & -2.62809241195645024e-04_dl, -2.25526230047976771e-04_dl, -1.97056496169645929e-04_dl, -9.48155565463569663e-05_dl, &
        & -3.76061332581253244e-05_dl, 1.62212450221678627e-06_dl, 1.17430468736664804e-05_dl, 1.67560571605375711e-05_dl, &
        & 9.56789853104267964e-06_dl, 1.77451576732129805e-06_dl, -2.35412713272733512e-06_dl, -5.93462095222011388e-06_dl, &
        & -5.25318654211783715e-06_dl, -4.27249213037148430e-06_dl, -6.88227990623243845e-06_dl, -4.91601818453864511e-06_dl, &
        & -3.73359146546522097e-07_dl, -3.45513306477439251e-06_dl, -4.02847060851322124e-06_dl, -3.74748655352561035e-07_dl, &
        & -3.63575787357239369e-07_dl, -1.55381019085711663e-06_dl, 5.79190647934334481e-08_dl, -8.06192546036142227e-07_dl, &
        & 1.44786325477362255e-06_dl, 2.07388293122767629e-06_dl, -5.39733939012545165e-07_dl, 1.61080130561079765e-07_dl, &
        & 4.56493216364450568e-06_dl /)

    if (a <= amin_blend) then
        ratio = 1._dl
        return
    end if
    t = max(amin_fit, min(1._dl, a))
    x = 2._dl*(log(t)-log_amin)/(-log_amin)-1._dl
    b1 = 0._dl
    b2 = 0._dl
    do j = 36, 1, -1
        b0 = 2._dl*x*b1-b2+c(j)
        b2 = b1
        b1 = b0
    end do
    raw = exp(x*b1-b2+c(0))
    if (a < amin_fit) then
        s = (a-amin_blend)/(amin_fit-amin_blend)
        s = s*s*(3._dl-2._dl*s)
        ratio = 1._dl+s*(raw-1._dl)
    else
        ratio = raw
    end if
    end function locked_h_ratio
    ...
    real(dl), external :: locked_h_ratio
    ...
    if (grhoa2 <= 0) then
        ...
        dtauda = sqrt(3 / grhoa2) / locked_h_ratio(a)
    end if
    ...
    real(dl), save :: sat_max_deltaC = 0._dl, sat_max_vC = 0._dl
    real(dl), save :: sat_max_deltaphi = 0._dl, sat_max_vphi = 0._dl
    logical, parameter :: use_theory_sat = .true.
    include 'sat_locked_background.inc'
    ...
    type EvolutionVars
        ...
        integer sat_ix !Index of four dynamical saturation perturbation variables
    ...
    subroutine sat_background_at(a, hbar, Cbg, ubg, phibg, vbg, rhoC, VC, VP, VCC, VPP)
    implicit none
    real(dl), intent(in) :: a
    real(dl), intent(out) :: hbar, Cbg, ubg, phibg, vbg, rhoC, VC, VP, VCC, VPP
    integer :: lo, hi, mid
    real(dl) :: x, x0, x1, w, act
    ...
    if (a <= sat_a(1)) then
        ...
        VCC = sat_VCC(1)
        VPP = sat_VPP(1)
    elseif (a >= sat_a(sat_bg_n)) then
        hbar = sat_hbar(sat_bg_n)
        Cbg = sat_Cbg(sat_bg_n)
        ubg = sat_ubg(sat_bg_n)
        phibg = sat_phibg(sat_bg_n)
        vbg = sat_vbg(sat_bg_n)
        rhoC = sat_rhoC(sat_bg_n)
        VC = sat_VC(sat_bg_n)
        VP = sat_VP(sat_bg_n)
        VCC = sat_VCC(sat_bg_n)
        VPP = sat_VPP(sat_bg_n)
    else
        lo = 1
        hi = sat_bg_n
        do while (hi-lo > 1)
        ...
        x1 = log(sat_a(hi))
        w = (x-x0)/(x1-x0)
        hbar = (1._dl-w)*sat_hbar(lo) + w*sat_hbar(hi)
        Cbg = (1._dl-w)*sat_Cbg(lo) + w*sat_Cbg(hi)
        ubg = (1._dl-w)*sat_ubg(lo) + w*sat_ubg(hi)
        phibg = (1._dl-w)*sat_phibg(lo) + w*sat_phibg(hi)
        vbg = (1._dl-w)*sat_vbg(lo) + w*sat_vbg(hi)
        rhoC = (1._dl-w)*sat_rhoC(lo) + w*sat_rhoC(hi)
        VC = (1._dl-w)*sat_VC(lo) + w*sat_VC(hi)
        VP = (1._dl-w)*sat_VP(lo) + w*sat_VP(hi)
        VCC = (1._dl-w)*sat_VCC(lo) + w*sat_VCC(hi)
        VPP = (1._dl-w)*sat_VPP(lo) + w*sat_VPP(hi)
    end if

    ! CAMB is standard before the post-bounce splice. Switch the field sector
    ! on over the same 5e-5 to 1e-4 interval used by locked_h_ratio.
    if (a <= 1.39650000000000006e-04_dl) then
        act = 0._dl
    elseif (a >= 2.10000000000000009e-04_dl) then
        act = 1._dl
    else
        act = (a-1.39650000000000006e-04_dl)/(2.10000000000000009e-04_dl-1.39650000000000006e-04_dl)
        act = act*act*(3._dl-2._dl*act)
    end if
    ubg = act*ubg
    vbg = act*vbg
    VC = act*VC
    ...
    VCC = act*VCC
    VPP = act*VPP
    end subroutine sat_background_at

    subroutine sat_scaled_background(a, adotoa, Cdot, Pdot, VCc, VPc, VCCc, VPPc)
    implicit none
    real(dl), intent(in) :: a, adotoa
    real(dl), intent(out) :: Cdot, Pdot, VCc, VPc, VCCc, VPPc
    real(dl) :: hbar_m, Cbg_m, ubg_m, phibg_m, vbg_m, rhoC_m
    real(dl) :: VC_m, VP_m, VCC_m, VPP_m
    real(dl) :: Bnorm, R2, Anorm

    call sat_background_at(a, hbar_m, Cbg_m, ubg_m, phibg_m, vbg_m, rhoC_m, &
        VC_m, VP_m, VCC_m, VPP_m)
    Bnorm = State%grhov/sat_rhoC0
    if (abs(hbar_m) > 1.e-30_dl .and. a > 0._dl) then
        R2 = adotoa*adotoa/(a*a*hbar_m*hbar_m*sat_rhos)
    else
        R2 = 0._dl
    end if
    Anorm = sqrt(max(Bnorm*sat_rhos,0._dl))
    Cdot = a*Anorm*ubg_m
    Pdot = a*Anorm*vbg_m
    VCc = sqrt(max(Bnorm*R2,0._dl))*VC_m
    VPc = sqrt(max(Bnorm*R2,0._dl))*VP_m
    VCCc = R2*VCC_m
    VPPc = R2*VPP_m
    end subroutine sat_scaled_background

    subroutine sat_theory_derivs(a, k, adotoa, phi, phidot, eta, etadot, &
        deltaC, vC, deltaphi, vphi, deltaC_p, vC_p, deltaphi_p, vphi_p)
    implicit none
    real(dl), intent(in) :: a, k, adotoa, phi, phidot, eta, etadot
    real(dl), intent(in) :: deltaC, vC, deltaphi, vphi
    real(dl), intent(out) :: deltaC_p, vC_p, deltaphi_p, vphi_p
    real(dl) :: Cdot, Pdot, VCc, VPc, VCCc, VPPc
    real(dl) :: zsync

    ! The first metric argument carries CAMB's synchronous shear variable z.
    zsync = phi
    call sat_scaled_background(a, adotoa, Cdot, Pdot, VCc, VPc, VCCc, VPPc)
    deltaC_p = vC
    vC_p = -2._dl*adotoa*vC - k*zsync*Cdot - (k*k + a*a*VCCc)*deltaC
    deltaphi_p = vphi
    vphi_p = -2._dl*adotoa*vphi - k*zsync*Pdot - (k*k + a*a*VPPc)*deltaphi
    end subroutine sat_theory_derivs

    subroutine sat_theory_stress(a, k, adotoa, deltaC, vC, deltaphi, vphi, &
        delta_rho, delta_q, delta_p, pi_C)
    implicit none
    real(dl), intent(in) :: a, k, adotoa, deltaC, vC, deltaphi, vphi
    real(dl), intent(out) :: delta_rho, delta_q, delta_p, pi_C
    real(dl) :: Cdot, Pdot, VCc, VPc, VCCc, VPPc, dV

    call sat_scaled_background(a, adotoa, Cdot, Pdot, VCc, VPc, VCCc, VPPc)
    dV = VCc*deltaC + VPc*deltaphi
    delta_rho = Cdot*vC + Pdot*vphi + a*a*dV
    delta_q = k*(Cdot*deltaC + Pdot*deltaphi)
    delta_p = Cdot*vC + Pdot*vphi - a*a*dV
    pi_C = 0._dl
    end subroutine sat_theory_stress
    ...
    yout(EVOut%sat_ix:EVOut%sat_ix+3) = y(EV%sat_ix:EV%sat_ix+3)
    ...
    real(dl) drho_sat, dq_sat, dp_sat, pi_sat
    real(dl), external :: locked_h_ratio
    ...
    if (State%flat) then
        adotoa=sqrt(grho/3)*locked_h_ratio(a)
        cothxor=1._dl/tau
    else
        adotoa=sqrt((grho+State%grhok)/3._dl)*locked_h_ratio(a)
    ...
    dp_sat = 0._dl
    pi_sat = 0._dl
    if (use_theory_sat) then
        call sat_theory_stress(a, k, adotoa, ay(EV%sat_ix), ay(EV%sat_ix+1), &
            ay(EV%sat_ix+2), ay(EV%sat_ix+3), drho_sat, dq_sat, dp_sat, pi_sat)
        dgrho = dgrho + drho_sat
        dgq = dgq + dq_sat
    end if
    ...
    call sat_theory_derivs(a, k, adotoa, z, 0._dl, ay(ix_etak), ayprime(ix_etak), &
        ay(EV%sat_ix), ay(EV%sat_ix+1), ay(EV%sat_ix+2), ay(EV%sat_ix+3), &
        ayprime(EV%sat_ix), ayprime(EV%sat_ix+1), ayprime(EV%sat_ix+2), ayprime(EV%sat_ix+3))

    sat_max_deltaC = max(sat_max_deltaC, abs(ay(EV%sat_ix)))
    sat_max_vC = max(sat_max_vC, abs(ay(EV%sat_ix+1)))
    sat_max_deltaphi = max(sat_max_deltaphi, abs(ay(EV%sat_ix+2)))
    sat_max_vphi = max(sat_max_vphi, abs(ay(EV%sat_ix+3)))
    if (a > 0.9999_dl) then
        open(unit=97, file='/mnt/data/theory_interface_field_maxima.txt', status='unknown', action='write')
        write(97,'(4ES24.14)') sat_max_deltaC, sat_max_vC, sat_max_deltaphi, sat_max_vphi
        close(97)
    end if
    ...
    if (associated(EV%OutputTransfer) .or. associated(EV%OutputSources)) then
        ...
        if (use_theory_sat) dgpi = dgpi + pi_sat
    ...
    adotoa=sqrt(grho/3._dl)*locked_h_ratio(a)
    adotdota=(adotoa*adotoa-gpres)/2
